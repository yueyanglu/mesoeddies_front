function [fu3d_cs,fv3d_cs] = csgrain_flds(grid,grid_cs,fu3d,fv3d,ifdecomp)
...
for ik = 1:nk
    ...
    if ifdecomp % decompose-based filtering
        ...
        fu_cs_h = sepblockfun(fu(2:end,:),[win_len,win_len],'nanmean');
        fu_cs = cat(1, zeros(1,size(fu_cs_h,2)),fu_cs_h);
        fu_cs(end,:) = 0;
        %
        fv_cs_h = sepblockfun(fv(:,2:end),[win_len,win_len],'nanmean');
        fv_cs = cat(2, zeros(size(fv_cs_h,1),1),fv_cs_h);
        fv_cs(:,end) = 0;
