...
% ntr = 5; % icomb = '6' for 12357; '12' for 12457
% carry_al = [1 2 3 4 5 6 7 8];

% ntrr=2: icomb =15 for C29; 
% ntr=3: icomb =22 for C159; 
% ntr=5 icomb =7 for C12357; =17 for C12457 
% ntr = 5; 
...
% ntr = 3; % icomb = '6' for 12357; '12' for 12457
% carry_al = [ 4 6 9];
...
%-------- dir for  
tmStr = '_180d';
efStr = '_prog_addreld10'; % _uvhm or '_prog', '_prog_addreld10', '_uvhm_addreld10'
eforc_varnm = 'cforc_rel'; % cforc__rel, cforc_m2,...
...
for icomb = 1:ncomb
    ...
    %------------------------dir for saving ------------------------
    save_dir = [save_root_dir 'lmd_ka' kaStr usetrStr efStr ...
    ...
    for it = 1:nt_al
        ...
        if any(carries == 11)
            ds_tr_cfc = ncstruct(tr_cfc_fnm);
            ds_eforc_cfc = ncstruct(eforc_cfc_fnm);
            fprintf(1,'Read <c_CFC> from: %s\n', tr_cfc_fnm);
            fprintf(1,'Read EF for c_CFC from: %s\n', eforc_cfc_fnm);
        end
        %----------------------- each layer
        [lmdu_z,lmdv_z] = deal(zeros(grid.nih,grid.njh,nk));
        ...
        for ik = 1:nk
            ...
            [hcx_p,hcy_p, hcxx,hcyy] = deal(zeros(grid.nih,grid.njh,ntr));
            ...
            for itr = 1:ntr
                ...
                elseif wichtr == 11
                    wichtr = 1;
                    ds_eforc_do = ds_eforc_cfc;
                    ds_tr_do = ds_tr_cfc;
                    fprintf(1,'Read CFC (tr11).\n');
                elseif wichtr <= 8 
            ...
            lhs = - eforc;
            % --- subtract the ka ----
            lhs_ka = - k11 .* hcxx - k22 .* hcyy;
            lhs = lhs - lhs_ka;
            %
            fprintf('\nCalc Lambda at Z%02d...\n', ik);
            [lmdu,lmdv] = flx2Lambda_MOM(lhs,hcx_p,hcy_p);
