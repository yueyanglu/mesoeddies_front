program solve_trac_main
  ...
  !
  IC_tr_fnm = trim(forc_dir)//'ICs/trac_CFC11_64.nc'
  ! IC_tr_fnm = trim(forc_dir)//'tr_patch_CS/tr__0021_060_00.nc'
  flx_dir = trim(forc_dir)//'uvhm_CS_decomp'//trim(tmstr)//'/'  
  h_dir = trim(forc_dir)//'sol_h'//trim(tmstr)//'/' 
  eforc_dir = trim(exp_dir)//'cforc_cfcrel125d_prog_addreld10/'
  flnmrel_ful = trim(forc_dir)//'ICs/trac_CFC11_64.nc'; relnm = 'tr1'
  !
  ...
  flnm_chidotn_ful = trim(chidotn_dir)//'chinxyz_closure1d_alpha25.nc'
  print*, 'Time-indepent Chi*n will be read from: ', trim(flnm_chidotn_ful)

  dir_save = trim(exp_dir)//'sols_cfc11/tr'//trim(advStr)&
              //trim(forcStr)//'_paramka400Chidotn_chinclosure1d_alpha25/'
  ! prog_addrel10d_norel/
  
  reldir_save = trim(dir_save)//'relforc/'
  T_relx = 125. * dy2hr*hr2sc ! 1day in [sec]
